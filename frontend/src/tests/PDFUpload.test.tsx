...
describe('PDFUpload', () => {
  ...
  it('handles file input change with valid PDF', async () => {
    ...
    const input = screen.getByLabelText('Choose PDF File').querySelector('input') as HTMLInputElement;
  ...
  it('shows error notification for non-PDF files', async () => {
    const user = userEvent.setup();
    renderWithProviders(<PDFUpload />);
    ...
    expect(mockShowNotification).toHaveBeenCalledWith('Please select a PDF file', 'error');
  });

  it('handles drag and drop events', async () => {
    renderWithProviders(<PDFUpload />);

    const dropZone = screen.getByText('Drop PDF files here or click to browse').closest('div') as HTMLElement;

    // Test drag enter
    fireEvent.dragEnter(dropZone);
    expect(dropZone.closest('.MuiPaper-root')).toHaveStyle('border: 2px dashed #1976d2');

    // Test drag leave
    fireEvent.dragLeave(dropZone);
    expect(dropZone.closest('.MuiPaper-root')).toHaveStyle('border: 2px dashed #ccc');
  });
  ...
  it('shows loading state during upload', () => {
    ...
    expect(screen.getByText('Choose PDF File')).toBeDisabled();
  });
  ...
  it('shows upload dialog with correct file information', async () => {
    ...
    const file = new File(['pdf content'], 'large-document.pdf', { type: 'application/pdf' });
